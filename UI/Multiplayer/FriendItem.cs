...
namespace Gameplay.Script.UI.Multiplayer
{
    public class FriendItem : MonoBehaviour
    {
        ...
        private Action<User> _onClick;
        private User _user;

        public void InitItem(User user, Action<User> onClick)
        {
            _user = user;
            _onClick = onClick;
            nickname.text = user.DisplayName;
            status.text = user.PresenceStatus.ToString();
            StartCoroutine(LoadTexture(user.SmallImageUrl, tex => head.texture = tex));
        }

        public void OnClickFriend()
        {
            _onClick?.Invoke(_user);
        }
